...
import { Users, Home, Plus, TrendingUp, DollarSign, Bell } from 'lucide-react';

export default function Dashboard({ params: _params }: { params: { slug: string } }) {
  const stats = [
    {
        ...
        {stats.map((stat) => (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div
                className={`${stat.bg} ${stat.darkBg} ${stat.color} ${stat.darkColor} p-3 rounded-lg`}
              >
                <stat.icon size={24} />
              </div>
